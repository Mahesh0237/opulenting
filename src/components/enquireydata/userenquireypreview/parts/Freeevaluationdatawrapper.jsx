...
function Freeevaluationdatawrapper({ freEvaluationData }) {
    const { userid } = useContext(UserAuth)
    const db = getDatabase(app)
    const updateStatus = (id, value) => {
        set(ref(db, `users/${userid}/user_enquirey_data/free_evaluation/${id}/status`), {
            status: value
        })
            .then((res) => {
                notifications.show({
    ...
    const deleteData = (id) => {
        openConfirmModal({
            ...
            onConfirm: () => {
                const dataRef = ref(db, `users/${userid}/user_enquirey_data/free_evaluation/${id}`)
                remove(dataRef)
                    .then((res) => {
                        notifications.show({
                            title: 'notification',
                    ...
                    <tbody>
                        {
                            freEvaluationData !== null ?
                                Object.keys(freEvaluationData).map((id) => {
                                    const userData = freEvaluationData[id]
                                    return (
                                        <tr key={id}>
                                            <td>{userData.name}</td>
...
export default Freeevaluationdatawrapper