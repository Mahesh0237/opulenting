...
function Walkindatawrapper({ walkinData }) {
    const { userid } = useContext(UserAuth)
    const db = getDatabase(app)
    const updateStatus = (id, value) => {
        set(ref(db, `users/${userid}/user_enquirey_data/walkin/${id}/status`), {
            status: value
        })
            .then((res) => {
                notifications.show({
    ...
    const deleteData = (id) => {
        openConfirmModal({
            ...
            children: (
                <Text size="sm">Are you sure you want to delete? This action is destructive and you can't able to restore the deleted data. </Text>
            ),
            labels: { confirm: 'Delete', cancel: "No don't delete it" },
            onConfirm: () => {
                const dataRef = ref(db, `users/${userid}/user_enquirey_data/walkin/${id}`)
                remove(dataRef)
                    .then((res) => {
                        notifications.show({
                            title: 'notification',
            ...
            <ScrollArea w="100%" scrollbarSize={6}>
                <Table withBorder withColumnBorders verticalSpacing="sm" >
                    ...
                    <tbody>
                        {
                            walkinData !== null ?
                                Object.keys(walkinData).map((id) => {
                                    const userData = walkinData[id]
                                    return (
                                        <tr key={id}>
                                            <td>{userData.name}</td>
...
export default Walkindatawrapper